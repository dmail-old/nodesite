...
(function(){
	...
	*/
	...
	var measurers = {
		...
		cumulativeOffset: function(axis){
			...
			while(element){
				...
				if( element != this && (NS.browser.safari || NS.browser.firefox && !borderBox(element)) ){
	...
	if( 'getBoundingClientRect' in Element.prototype && !NS.os.ios ){
		measurers.offsets = function(axis){
			var html, offset;

			html = this.getDocument().documentElement;
			offset = parseInt(this.getBoundingClientRect()[axis == 'x' ? 'left' : 'top'], 10);
