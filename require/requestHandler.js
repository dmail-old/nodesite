var FileResponse = new Class({
	...
	stream: function(){
		...
		this.streaming = file.readStream();
...
var Page = {
	...
	parseMeta: function(name, value){
		...
		var attr = 'name', str = '<meta {attr}="{name}" content="{value}" />', meta;
...
function PageResponse(response){
	...
	var htmlFile = new File(root + '/app.html');
	
	try{	
		var html = String(htmlFile.readSync());
	}catch(e){
		return serveError(e);
	}
...
var AjaxResponse = new Class({
	...
	sendScriptResponse: function(path, args){
		...
		applyScript(path, this, args, function(error, response){
