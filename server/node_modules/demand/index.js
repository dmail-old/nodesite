...
// logger
demand.use('logger', function(next){
	this.emitter.on('header', this.log);
	next();
});
...
demand.use('sendFile', function(next){
	...
	function isFileRequest(){
		...
		if( dirname == 'css' || dirname == 'js' || dirname == 'img' || dirname == 'element' ){
			return true;
		}
	...
	if( isFileRequest.call(this) ){
		this.sendFile(global.CLIENTPATH + this.url.pathname);
	}
	else{
		next();
	}
