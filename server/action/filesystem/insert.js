	...
	if( data.type == 'file' ){
		...
		FS.mkdir(path + '/' + data.name, data.mode || mode, function(error){
			...
			if( typeof childNodes == 'undefined' ){
				...
				if( !(childNodes instanceof Array) ) return callback(new TypeError('directory children muste be an array'));

				function nextChild(error){
					if( error ) return callback(error);
					// tout les enfants ont été créés
					if( childNodes.length === 0 ) return callback();

					global.callScript(__filename, this, path + '/' + data.name, childNodes.shift(), nextChild);
				};
				nextChild();
			}
		});
