...
var Profile = proto.create({
	...
	callback: function(result){
		...
		var methodCalls = methods.map(function(method){
			...
			totalDuration+= Math.round(durations.sum(), 0);

			return [
				method.name,
				method.calls.length,
				0,
				Math.round(durations.sum(), 0) + 'ms',
				Math.round(durations.average(), 0) + 'ms',
				Math.round(durations.min(), 0) + 'ms',
				Math.round(durations.max(), 0) + 'ms',			
				basedirname(method.file),
				method.line
			];
		});

		methodCalls.forEach(function(methodCall){
			methodCall[2] = Math.round(100*(parseInt(methodCall[3].slice(0, -2), 10) / totalDuration), 0);
		});

		methodCalls = methodCalls.sort(function(callA, callB){
			return callB[2] - callA[2];
		});

		var Table = require('TableTemplate');
		var table = new Table();

		table.addRow(['Function', 'Calls', 'Percent', 'Duration', 'Avg', 'Min', 'Max', 'File', 'Line']);
