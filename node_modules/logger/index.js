...
CECI DEVRAIT ETRE UN DUPLEX STREAM
...
var Logger = {
	...
	write: function(level, message, data){
		...
		for(;i<j;i++){
			...
			if( stream.isTTY ){
				stylized = stylized || this.render(level, message, data, true);
				stream.write(stylized);
			}
			// and raw data for other streams
			else{
				raw = raw || this.render(level, message, data, false);
				stream.write(raw);
			}
		}
