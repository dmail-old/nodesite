...
var AppTest = TestSerie.create({
	...
	loadTests: function(){
		var testLoader = this.TestLoader,
		badSignaturePaths = this.badSignaturePaths, modulePaths;

		modulePaths = this.ModuleLoader.load(this.path, badSignaturePaths);
		if( badSignaturePaths && badSignaturePaths.length ){
			this.badSignaturePaths = badSignaturePaths.filter(testLoader.filterFolder, testLoader);
