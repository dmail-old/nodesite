...
var LogGenerator = require('TemplateGenerator');
var LineLog = Log.new(LineLogTemplate).registerStyles(LineLogStyles);
var LineLogGenerator = LogGenerator.new(LineLog, LineLogSeparator, function(lineLog, value, index){
	lineLog.exec({
		lineNumber: index,
		lineSource: value
	});
	...
	if( index == error.line ){
		lineLog.globalStyle = 'currentLine';
		lineLog.registerStyle('lineNumber', 'currentLineNumber');
	}
...
appTest.listener = {
	...
	'test-fail': function(e){
		...
		if( error.name == 'AssertionError' ){
			...
			// il faut préparer un résultat sur lequel on peut ensuite appeler toString, toStylizedString
			// ce résultat doit créer une instance de kkchose, de sorte qu'on puisse créer plusieurs résultat
			// Pour les log plus tard
			var source = LineLogGenerator.createResult(lines);
