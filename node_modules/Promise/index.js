...
Promise.iterate = function(iterable, thenMap, thenMapBind, collectFn, collectBind, initialValue){
	var promise, index = 0;

	promise = Promise.resolve(initialValue);

	if( thenMap && typeof thenMap != 'function' ) throw new TypeError('thenMap mus be a function');
	...
	forOf(iterable, function(value){
		...
		if( collectFn ){
			promise.then(collectFn.bind(collectBind));			
		}
	});
...
Promise.reduce = function(iterable, bind, initialValue){
	return Promise.iterate(iterable, reducer, bind, null, null, initialValue);
};
...
Promise.serie = function(iterable, bind, initialValue){
	var values = [];
	return Promise.iterate(iterable, reducer, bind, collector, values, initialValue).then(function(){
		return values;
	});
};
...
Promise.map = function(iterable, map, bind){
	var values = [];
	return Promise.iterate(iterable, function(value, index, iterable){
		return map.bind(bind, value, index, iterable);
	}, null, collector, values).then(function(){
		return values;
	});
};
...
if( typeof topLevelScope['Promise'] == 'undefined' ){	
	topLevelScope.Promise = Promise;
}