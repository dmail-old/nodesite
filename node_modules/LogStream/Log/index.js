var ConsoleTemplate = require('./ConsoleTemplate');
var proto = require('proto');

var Log = proto.extend.call(ConsoleTemplate, {
	level: null,
	suffix: '\n',
	mergePropertyNames: ConsoleTemplate.mergePropertyNames.concat('level'),

	constructor: function(string, level){
		//if( typeof level != 'string' ) throw new TypeError('Log must define a level');
		this.level = level;
		Log.super.constructor.call(this, string);
	},

	toString: function(){
		var output;

		output = this.level && this.level != 'log' ? this.level + ':' : '';
		output+= Log.super.toString.call(this);
	...
	toStylizedString: function(){
		...
		output = this.level && this.level != 'log' ? this.stylizeValue(this.level, this.level) + ': ' : '';
		output+= Log.super.toStylizedString.call(this);

		return output;
	},

	toJSON: function(){
		var obj = Log.super.toJSON.call(this);
		obj.level = this.level;
		return obj;
	}
});

Log.prototype.registerStyles({
	//'log': {},
	'info': ['green'],
	'help': ['cyan'],
	'warn': ['yellow', 'bold'],
...
var InspectLog = proto.create({
	...
	constructor: function(object, styles){
		this.object = object;
	},
