...
exports['complete to a task'] = function(test, Task){
	test.resolveTo(Task.complete(Task.complete('foo')), 'foo');
};
...
exports['fail by consumer'] = function(test, Task){
	...
	test.resolveTo(task.catch(function(error){
		return error;
	}), 'bar');
};

exports['cancel prevent completion'] = function(test, Task){
	test.resolveTo(Promise.race([
...
exports['pause prevent completion & resume restore state'] = function(test, Task){
	...
	test.resolveIn(task.then(function(){ return new Date(); }), 100);
};
...
exports['pause propagate pausing to consumer & resume restore state'] = function(test, Task){
	...
	test.resolveIn(subTask.then(function(){ return new Date(); }), 100);
};

exports['delay'] = function(test, Task){
	test.resolveIn(Task.complete(10).delay(100).then(function(){ return new Date(); }), 100);
};

exports['delay return value'] = function(test, Task){
	test.resolveTo(Task.complete(10).delay(100), 10);
};
...
exports['timeout'] = function(test, Task){
	test.resolveTo(Task.complete(10).delay(100).timeout(10).catch(function(error){
		return error.code;
	}), 'TASK_TIMEOUT');
};
...
exports['all pausing / resuming'] = function(test, Task){
	...
	test.resolveIn(task.then(function(){ return new Date(); }), 100, 40);
};
...
exports['return task'] = function(test, Task){
	test.resolveTo(Task.complete(10).then(function(){
		return Task.complete('bar');
	}), 'bar');
};