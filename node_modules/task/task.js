/*
Il serait surement possible de supporter previousList et non previous
Lorsqu'une tâche se complète, les tâches suivante ne sont lancées que si
toute leur tâches précédentes sont complètes, c'est a priori facile à faire et permettais ceci:
si une des previous fail, le tâche fail
c'est la comportement actuel de all à la différence que then() est appelé avec un tableau

var A = new Task(), B = new Task();

var C = Task.join(A, B, function(aValue, bValue){ return aValue + bValue; });

qui ferais donc en interne
...
var Task = proto.extend({
	...
	progress: function(ancestorTask){
		...
		this.notify('progress', ancestorTask);
		this.propagateProgress(ancestorTask);
	},

	propagateProgress: function(ancestorTask){
		if( this.nextList ){
			this.nextList.forEach(function(next){
				this.debug('propaging the completion of', ancestorTask, 'as progress for', next);
				next.progress(ancestorTask);
			}, this);
		}
	},
	...
	propagate: function(method){
		...
		var direction = this.propagationDirection, left = false, right = false;
	...
	chain: function(task, detach){
		if( task.previous ){
			throw new Error('cannot chain task : multiple dependency not supported and ' + task + ' already depends on ' + task.previous);
		}
