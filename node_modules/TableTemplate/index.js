...
var CellWidthManager = proto.create({
	...
	getCellWidth: function(cell){
		...
		else if( this.isPercentage(styleWidth) ){
			width = this.percent(styleWidth, this.availableWidth);
		}
	...
	autoWidth: function(){
		...
		if( diff > 0 ){
			...
			columns.forEach(function(column, index){
				...
				if( width < this.minWidth ){
					decreaseAmount = maxCellWidth - this.minWidth;
					width = this.minWidth;
				}

				remainingOverflow-= decreaseAmount;
				debug('decreasing column width n°', index, 'from', dimensions.width, 'to', width,
					'(', decreaseAmount, ')');
		...
		columns.forEach(function(column, index){
			debug('setting column n°', index, 'width to', column.meta('dimensions').width);
...
var TableTemplate = proto.extend(Table, {
	...
	toString: function(){
		...
		this.manager.minWidth = this.cellStyle.minWidth;
		this.manager.computeWidths();
