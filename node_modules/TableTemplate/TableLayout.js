...
var TableLayout = proto.create({
	...
	constructor: function(tableTemplate){
		...
		this.width = toSize(tableTemplate.style.width, this.availableWidth);
	...
	autoWidth: function(columns){
		...
		var diff = this.desiredWidth - this.width;
		...
		if( diff > 0 ){
			...
			resizableColumns.forEach(function(column, index){
				...
				debug('decreasing column width n°', index, 'from', oldWidth, 'to', column.width, '(', decreasedAmount, ')');
			}, this);
		...
		// the colmuns are too small, we increase their size proportionnaly
			...
			resizableColumns.forEach(function(column, index){
				...
				debug('increasing column width n°', index, 'from', oldWidth, 'to', column.width, '(', increasedAmount, ')');
			});
