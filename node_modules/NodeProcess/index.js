...
var prepareArgs = require('prepareProcessArgs');
...
var NodeProcess = proto.extend(Emitter, {
	...
	constructor: function(path, args){
		Emitter.call(this);
		...
		try{
			configPath = require.resolve(path + '.config.js');
		}
		catch(e){
			// no config file
		}
	...
	onFileChange: function(path){
		...
		if( path === this.configPath ){
			// il faut surtout clean les events etc
			this.unsetConfig();
			// restart asap but not immediatly because
			// dependening on the env, an antivirus maybe scanning the file
			// because it was modified recently, thus locking access to config file
			setTimeout(this.restart.bind(this), 100);
			// error.code == 'EBUSY';
			// faudrais checker toutes les 10ms qu'on obtient EBUSY, avec un délai max d'attente
		}
		else{
			this.restart();
		}
	...
	start: function(){
		...
		var preparedArgs = prepareArgs(this.args);
