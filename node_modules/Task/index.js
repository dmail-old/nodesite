...
var TaskSerie = proto.extend(Task, {
	...
	recover: function(){
		// this taskserie has failed, it's necessarily because of this.current task
		if( this.isFailed() && this.current ){
			...
			// the current task is TaskSerie, it necessarily failed because of a deeper Task
			if( this.current instanceof TaskSerie ){
				this.current.recover();
			}
			// it's a classic Task, we try to restart this task to let this taskSerie a chance to resolve 
			else{
				this.current.restart();
			}
		}
