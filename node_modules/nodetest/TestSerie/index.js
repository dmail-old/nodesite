...
var TestRunner = proto.create({
	...
	*/
	...
	pauseUntilTaskIsCompleted: function(taskA, taskB){
		// pause the task
		taskA.pause();
		taskB.then(taskA.resume.bind(taskA));
		taskB.start();
	},

	pauseUntilReload: function(task, test){
		// delete cache
		delete require.cache[module.path];
		this.pauseUntilTaskIsCompleted(task, this.createTask('load', test));
	},

	moduleChanged: function(module){
		var task = this.currentTask;
		
		// module changed while a module is being loaded
		if( task.name === 'load' && task.test instanceof ModuleTest ){
			// we are loading the changed module, restart
			if( task.test === module ){
				task.restart();
			}
			// we are loading a module after this one, meaning this module is already loaded
			else if( this.modules.indexOf(task.test) > this.modules.indexOf(module) ){
				this.pauseUntilReload(task, module);
			}
		}
		// module changed while an other task is running
		else if( task.isPending() ){
			// mettons que la tâche en cours c'était de run un test de ce module
			// il faudrais annuler tous les tests de ce module et les relancer
			// hors ce n'est pas le cas puisque currentTask 
			// vaut un test en particulier
			...
			task = Task.serie([this.createTask(module, 'load'), this.createTask(module, 'run')]);				
	...
	loadModule: function(module){
		return this.createTask('load', module);
	},
	...
	loadModuleFile: function(file){
		return this.createTask('load', file);
	},
