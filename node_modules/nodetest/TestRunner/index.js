...
var Assertion = proto.extend({
	...
	setErrorProperties: function(error){
		...
		else if( type == 'willResolve' ){
			error.message = 'promise expecting to resolve is rejected with ' + args[0];
		}
		else if( type == 'willBe' ){
			error.message = 'promise expected to resolve to '+ args[1] + ' but got ' + args[2];
		}
...
var Test = proto.extend({
	...
	willResolve: function(promise){
		...
		var assertion = this.createAssertion('willResolve', [promise]);
		this.assertions.push(assertion);
		this.pendingCount++;
		...
		promise.then(
			...
			function(value){
				...
				this.reject(assertion.createError());				
			}.bind(this)
		);
	},
