...
var TestReporter = proto.extend({
		...
		if( typeof listener == 'function' ){
			...
			this.listener = function(error){
				if( this.test.type === 'folder' ){
					...
					this.handleFileResult(this.test);
				}
				else{
	...
	handleFolderResult: function(folderTest){
		...
		table.column('tests').setStyle('textAlign', 'center');
		table.column('duration').setStyle('textAlign', 'center');
	...
	handleModuleResult: function(moduleTest){
		if( moduleTest.testFiles.length === 1 ) return this.handleFileResult(moduleTest.testFiles[0]);
	...
	handleFileResult: function(fileTest){
		var message = 'File test results {path} ({duration}ms {count} tests)\n{results}';
		var table = this.Table.create();

		table.addRow(['state', 'name', 'assertions', 'duration']);
		...
		table.column('assertions').setStyle('textAlign', 'center');
		table.column('duration').setStyle('textAlign', 'center');
		table.column('duration').format(function(value){
			return value + 'ms';
		});
...
var nodetest = {
	testFolder: function(path, fn, bind, watch){
		...
		return TestReporter.create(runner.testFolder(path), fn, bind, watch);
	},

	testModule: function(path, fn, bind, watch){
		path = require.resolve(path);
		return TestReporter.create(runner.testModule(path), fn, bind, watch);
	},
