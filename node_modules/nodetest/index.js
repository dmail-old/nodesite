...
var TestReporter = proto.create({
	handleFolderResult: function(meta){
		// il faudrais un truc genre testFolder.printDurations()
		// mais par défaut on map sur les tests
		/*

	table.widths = ['90%', '10%'];
		table.headers = ['module path', 'test count'];
		table.body = meta.moduleMetas.map(function(meta){
			return [meta.path, meta.fileMetas.reduce(function(previous, meta){
		...
		*/
		...
		//table.widths = ['70%', '30%'];
		/*
		table.headers = ['step', 'duration'];
		table.body = [
			['find', meta.findDuration],
			['load', meta.loadDuration],
			['require', meta.requireDuration],
			['test', meta.execDuration]
		];
		*/
