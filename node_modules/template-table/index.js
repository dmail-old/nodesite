...
var CellTemplate = proto.extend.call(Table.Cell, {
	ansi: require('ansi'),
	...
	constructor: function(){
		CellTemplate.super.constructor.apply(this, arguments);
		this.style = Object.create(this.style);
	},

	format: function(){
		var data = CellTemplate.super.format.call(this);

		if( !this.fixed && this.conditionalFormatting ){
			this.textFormat = this.conditionalFormatting.call(this, data);
		}
...
var CellRangeTemplate = proto.extend.call(Table.CellRange, {
	constructor: function(){
		CellRangeTemplate.super.constructor.apply(this, arguments);
		//this.style = Object.create(this.style);
	},
	...
	setMinWidth: function(minWidth){
		return this.setStyle('minWidth', minWidth);
	},

	conditionalFormat: function(conditionalFormatting){
		return this.set('conditionalFormatting', conditionalFormatting);
	}
});

var TableTemplate = proto.extend.call(Table, {
	Cell: CellTemplate,
	...
	constructor: function(cells){
		TableTemplate.super.constructor.call(this, cells);
	...
	drawCell: function(x, y){
		...
		if( contentWidth === width ){
			...
			if( textAlign === 'left' ){
				...
				content = cell.value.pad(width - 2, ' ');
			}
		}
		...
		if( cell.textFormat ){
			content = this.ansi.format(content, cell.textFormat);
		}
	...
	drawTable: function(){
		...
		for(;y<height;y++){
			...
			if( y != height - 1 ){
				table+= '\n';
				if( y === 0 && this.className != 'compact' && this.cells[0][0].fixed === true ){
	...
	toString: function(){
		if( this.cells.length ){
			this.compile();
			var layout = TableLayout.create(this);
...
TableTemplate.defineClass('border', 'grey');
TableTemplate.defineClass('header', 'cyan');
