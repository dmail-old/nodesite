...
AssertionFactory.register('willResolve', {
	...
	createPromise: function(){
		return this.args[0].catch(function(value){
			var error = this.createError();
			error.message = 'thenable expecting to resolve is rejected with ' + value;
			throw error;
		}.bind(this));
	}
});

AssertionFactory.register('resolveTo', {
	timeout: 100,
	...
	createPromise: function(){
		...
		return this.args[0].then(function(value){
			if( expected != value ){
				var error = this.createError();
				error.message = 'thenable expected to resolve to '+ expected + ' but got ' + value;
				throw error;
			}
		}.bind(this));
	}
...
AssertionFactory.register('remainPending', {
	timeout: 0,
	...
	createPromise: function(){
		...
		return Promise.race([
			...
			Promise.resolve(value).then(function(){
				...
				error.message = 'thenable supposed to remainPending has resolved';
