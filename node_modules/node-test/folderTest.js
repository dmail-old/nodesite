...
var ModuleCollector = require('module-util/collector-task');
...
var FolderTest = proto.extend({
	...
	react: function(method, task, value){
		if( method in this.reactions ){
			this.reactions[method].call(task, value);
		}
	},
	...
	exec: function(){
		...
		/*
		pour la progression il faut surement mettre en place un système
		pour éviter les collisions de noms comme :

		findModulePaths:collect:stat (c'est une sous-tâche de collect)
		findModulePaths:collect:readdir (c'est une sous-tâche de collect)
		findModulePaths:collect
		*/

		this.task.listen('progress', function(task, otherTask){
			//this.react('progress', otherTask);
			console.log(otherTask.name, 'completed making', task.name, 'progress');
		}, this);
...
FolderTest.tasks = [
	...
	function findModulePaths(path){
		...
		return ModuleCollector.create(path).createTask();
	},
	...
	function loadModuleTests(moduleTests){
		...
		return Task.map(moduleTests, function loadTests(moduleTest){
	...
	function loadModules(){
		...
		return Task.map(this.moduleTests, function loadModule(moduleTest){
			return moduleTest.load();
		});
	},
	// sort module by dependency
	function sortModules(){
		console.log('sorting modules', this.moduleTests.length);
	...
	// exec module tests
	function execTests(){
		console.log('executing tests', this.moduleTests.length);

		return Task.map(this.moduleTests, function execModuleTests(moduleTest){
