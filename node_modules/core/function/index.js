...
Function.implement = Object.implement.bind(Object, Function);
Function.complement = Object.complement.bind(Object, Function);
...
Function.argumentNames = function(fn){
	...
	var propertyName = 'argumentNames', source, declaration, args, i, j, names;

	if( propertyName in fn ){
		names = fn[propertyName];
	}
	else if( fn.length === 0 ){
		names = fn[propertyName] = [];
	}
		...
		function pushName(all, underscore, name){
			names.push(name);
		}
		...
		if( args ){
			...
			for(;i<j;i++){
				args[i].replace(RegExp.ARG, pushName);
			}
		}

		fn[propertyName] = names;
	}
...
index says where is the args array
