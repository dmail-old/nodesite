...
var app = {
	init: function(){
		...
		route.when('*', function(where){
			this.go(where);
		}, this);

		route.when('/example', function(){
			...
			/* ici, coté serveur je me retrouve avec l'url C:\Node\nodesite\server\client\example
			au lieu de C:\Node\nodesite\client\example
			je pense que ça vient du process.chdir
			ouais c'est ça, du coup je me demande ou je fous ces actions
			elles aussi appartiennent à la fois au client et au serveur et devrait donc sortir du dossier serveur?
			*/
			if( isDirectory ){
				server.callAction(
					'filesystem/readdir',
		...
		route.change(document.location.href);
	},

	setPage: function(html){
		if( typeof html != 'string' ) html = html.toString();

		$('page').innerHTML = html; // document.body.innerHTML = html;
	...
	go: function(url, state){
		...
		server.callAction('go', url, function(error, response){
	...
	popstate: function(e){
		route.change(document.location.href, e.state);
	},
	...
	click: function(e){
		...
		if( anchorElement ){
			...
			route.change(anchorElement.href);
			e.preventDefault();
			return false;
		}
	}
