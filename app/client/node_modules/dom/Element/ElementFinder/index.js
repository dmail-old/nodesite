var Element = require('../../Element');
var NodeFinder = require('document/nodeFinder');
var NodeInterface = require('document/nodeInterface');

Element.implement(NodeFinder);

if( typeof document != 'undefined' ){
	document.getFirst = document.html.getFirst.bind(document.html);
	document.getFirst = window.$$ = document.html.getFirst.bind(document.html);
}

if( !Element.prototype.compareDocumentPosition ){
	Element.prototype.compareDocumentPosition = NodeInterface.compareDocumentPosition;
}

// this function is meant to be used as comparer for array.sort
Element.comparePosition = function(a, b){
	if( !a.compareDocumentPosition || !b.compareDocumentPosition ) return 0;
	if( a === b ) return 0;
	if( a.compareDocumentPosition(b) & NodeInterface.FOLLOWING ) return -1;
	return 1;
};
