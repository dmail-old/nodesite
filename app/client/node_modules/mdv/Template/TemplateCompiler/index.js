...
Directives.parseAttribute = function(attrName, attrValue){
	var stringTemplate = StringTemplate.new(attrValue);
	...
	if( length === 1 ){
		...
		if( strings[0].length === 0 && strings[1].length === 0 ){
			return DirectLinker.new(attrName, expressions[1]);
		}
		// one mustache with prefix or suffix: "name:{name}!"
		else{
			return TokenLinker.new(attrName, strings[0], expressions[0], strings[1]);
		}
	}
	// more than one mustache : "Hello {{name}}, you are {{age}} years old"
	return TokenListLinker.new(attrName, stringTemplate);	
};
