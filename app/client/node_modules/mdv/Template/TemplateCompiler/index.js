...
Directives.parseAttribute = function(attrName, attrValue){
	var stringTemplate = StringTemplate.create(attrValue);
	stringTemplate.compile();
	var expressions = stringTemplate.expressions;
	var strings = stringTemplate.strings;
	...
	if( length === 1 ){
		// one mustache with no prefix and no suffix : "{name}"
		if( strings[0].length === 0 && strings[1].length === 0 ){
			return DirectLinker.create(attrName, expressions[1]);
		}
		// one mustache with prefix or suffix: "name:{name}!"
		else{
			return TokenLinker.create(attrName, strings[0], expressions[0], strings[1]);
		}
	}
	// more than one mustache : "Hello {{name}}, you are {{age}} years old"
	return TokenListLinker.create(attrName, stringTemplate);
};
...
Directives.parseAttributes = function(node){
	...
	return LinkerListLinker.create(linkerlist);
};
...
Directives.parseSubTemplate = function(node){
	if( mdv.isTemplate(node) ){
		return SubTemplateLinker.create(node);
	}
};
