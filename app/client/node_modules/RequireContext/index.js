/*
move this into require.page.js
and module.page.js
*/

var proto = require('proto');
var Resolver = require('./RequireResolver');
require('string/prototype/startsWith');

var RequireContext = proto.extend({
	Resolver: Resolver,
	publicFolder: null,
	cache: {},
	resolvedPaths : null,

	constructor: function(filename){
		filename = require('path').dirname(filename);

		if( filename in this.cache ){
			return this.cache[filename];
		}
		this.cache[filename] = this;
		this.filename = filename;
		this.resolvedPaths = {};
	},

	isClientReadable: function(){
		if( this.publicFolder && this.filename.startsWith(this.publicFolder) ) return true;
		if( this.meta && this.meta.clientReadable === true ) return true;
		return false;
	},

	resolve: function(path){
		if( path in this.resolvedPaths ) return Promise.resolve(this.resolvedPaths[path].path);

		var resolver = this.Resolver.create(this.filename, path);
		var p = resolver.resolve();

		return p.then(function(resolvedPath){
			this.resolvedPaths[path] = {
				path: resolvedPath,
				meta: resolver.packageBody
			};
			return resolvedPath;
		}.bind(this));
	},

	toJSON: function(){
		return {
			filename: this.filename,
			resolvedPaths: this.resolvedPaths
		};
	}
});

module.exports = RequireContext;
