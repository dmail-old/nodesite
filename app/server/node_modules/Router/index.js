...
var RequestHandler = proto.create({
	...
	createPromise: function(iterable, bind, initialValue){
		require('Symbol');
		...
		var promise = Promise.resolve(initialValue), isThen;

		var p = new Promise(function(resolve, reject){
			var iterator = iterable[Symbol.iterator](), next, fn;
				...
				if( next.done ){
					...
					if( isThen ){
						...
						promise = promise.catch(function(error){
							return fn.call(bind, initialValue, error);
						});
					}
	...
	next: function(error){
		...
		*/
		...
		var middlewares, middleware, handler, bind, index, ret;
		...
		if( typeof handler === 'function' ){
			if( error && handler.length !== 4 ) return this.next(error);

			// catch handler error
			try{
				debug('calling middleware ' + middleware.name);

				if( error ){
					ret = handler.call(bind, this.request, this.response, this, error);
				}
				else{
					ret = handler.call(bind, this.request, this.response, this);
				}
			}
			catch(e){
				// the error caused an other error
				if( error && error != e ){
					error.chidren = error.children || [];
					error.chidlren.push(e);
				}
				error = e;
			}
			...
			if( error || ret == null ){
				...
				Promise.resolve(ret).then(
					...
					function(reason){
						// a rejected handler without reason is not an error
