/*
https://github.com/substack/node-resolve/tree/master/lib
http://nodejs.org/api/modules.html#modules_all_together
*/

require('promise/from');
require('promise/mapFirst');
var proto = require('proto');
var debug = require('debug');

var RequireResolver = proto.create({
	Path: require('path'),
	fileSystem: require('fs'),

	folderName: 'node_modules',
	index: 'index',
	package: 'package.json',
	extension: '.js',

	cwd: null, // module filename
	requiredPath: null, // path required by a module
	rootFolder: null,
	packageBody: null,

	constructor: function(cwd, requiredPath, rootFolder){
		this.cwd = cwd;
		this.requiredPath = requiredPath;
		if( rootFolder ) this.rootFolder = rootFolder;
	},

	createNotFoundError: function(){
		var error = new Error('Cannot find module \''+ this.requiredPath +'\'');
		error.code = 'MODULE_NOT_FOUND';
		return error;
	},

	loadAsFile: function(path){
		debug('trying to load module at ', path);

		// devrait normalement essayer de load path + '.json' et path + '.node'

		// get file stat
		return Promise.from(function(complete){
			this.fileSystem.stat(path, complete);
		}, this).then(
			function(stat){
				// it's a file, return the path
				if( stat.isFile() || stat.isFIFO() ){
					debug('module found at', path);
					return path;
				}
				// it's something else
				throw this.createNotFoundError();
			}.bind(this),
			function(error){
				// file not found
				if( error.code === 'ENOENT' ){
					// try to find with extension
					if( this.Path.extname(path) != this.extension ) return this.loadAsFile(path + this.extension);
					throw this.createNotFoundError();
				}
				return Promise.reject(error);
			}.bind(this)
		);
	},

	loadAsDirectory: function(path){
		var packagePath = path + this.Path.sep + this.package;
		var indexPath = path + this.Path.sep + this.index;

		// read package.json
		return Promise.from(function(complete){
			this.fileSystem.readFile(packagePath, complete);
		}, this).then(
			function(json){
				var body = JSON.parse(json);
				this.packageBody = body;
				if( body.main ) return this.load(this.Path.resolve(path, body.main));
				return this.loadAsFile(indexPath);
			}.bind(this),
			function(error){
				// no package.json, load index.js
				if( error.code === 'ENOENT' ){
					return this.loadAsFile(indexPath);
				}
				return Promise.reject(error);
			}.bind(this)
		);
	},

	load: function(path){
		return this.loadAsFile(path).catch(function(error){
			if( error && error.code == 'MODULE_NOT_FOUND' ) return this.loadAsDirectory(path);
			return Promise.reject(error);
		}.bind(this));

		/*
		return Promise.first([
			this.loadAsFile,
			this.loadAsDirectory
		], this, path);
		*/
	},

	// http://nodejs.org/api/modules.html#modules_loading_from_node_modules_folders
	nodeModulePaths: function(start){
		/*
		1. let PARTS = path split(START)
		2. let ROOT = index of first instance of "node_modules" in PARTS, or 0
		3. let I = count of PARTS - 1
		4. let DIRS = []
		5. while I > ROOT,
			a. if PARTS[I] = "node_modules" CONTINUE
			c. DIR = path join(PARTS[0 .. I] + "node_modules")
			b. DIRS = DIRS + DIR
			c. let I = I - 1
		6. return DIRS
		*/

		var Path = this.Path;
		var parts = start.split(Path.sep), dirs, dir, i, root, part;

		root = parts.indexOf(this.folderName);
		if( root == -1 ) root = 0;

		i = parts.length - 1;
		dirs = [];
		while( i > root ){
			part = parts.pop();
			if( part == this.folderName ) continue;
			dir = Path.join(Path.join.apply(Path, parts), this.folderName);
			dirs.push(dir);

			// ne dois pas monter plus haut que root
			if( this.rootFolder && Path.dirname(dir) == this.rootFolder ) break;

			i--;
		}

		return dirs;
	},

	loadNodeModules: function(path, start){
		/*
		LOAD_NODE_MODULES(X, START)
		1. let DIRS=NODE_MODULES_PATHS(START)
		2. for each DIR in DIRS:
			a. LOAD_AS_FILE(DIR/X)
			b. LOAD_AS_DIRECTORY(DIR/X)
		*/

		var dirs = this.nodeModulePaths(start);

		if( dirs.length === 0 ) return Promise.reject(this.createNotFoundError());
		return Promise.mapFirst(dirs, function(directory){
			return this.load(directory + this.Path.sep + path);
		}, this);
	},

	resolve: function(){
		/*
		require(X) from module at path Y
		1. If X is a core module,
			a. return the core module
			b. STOP
		2. If X begins with './' or '/' or '../'
			a. LOAD_AS_FILE(Y + X)
			b. LOAD_AS_DIRECTORY(Y + X)
		3. LOAD_NODE_MODULES(X, dirname(Y))
		4. THROW "not found"
		*/

		var X = this.requiredPath;
		var Y = this.cwd;

		function cb(e, path){
			// si il s'agit d'une demande client et que le module est plus haut que app/node_modules
			// il doit faire partie d'une 'whitelist' de module qu'on envoie au client

		}

		if( X[0] == '/' || X.slice(0,2) == './' || X.slice(0,3) == '../' ){
			return this.load(this.Path.resolve(Y, X));
		}
		return this.loadNodeModules(X, this.Path.dirname(Y));
	}
});

module.exports = RequireResolver;
