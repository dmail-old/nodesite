...
var Module = proto.create({
	...
	rootPath: global.ROOT_PATH,
	cache: {},
	resolvedPaths : null,
	meta: null, // data from package.json file

	new: function(filename, parent){
		filename = this.Path.resolve(this.rootPath, filename);
		...
		if( filename in this.cache ){
			...
			module = proto.new.call(this, filename, parent);
	...
	isClientReadable: function(){
		if( this.filename.startsWith(global.APP_PATH) ) return true;
		...
		if( path in this.resolvedPaths ){
			...
			resolver.resolve(function(error, filename){
				if( error ){
					...
					this.meta = this.ModuleResolver.packageBody;
					// make filename relative to the root path
					filename = this.Path.relative(this.rootPath, filename);
