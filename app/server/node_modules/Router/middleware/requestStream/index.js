...
var RequestMethods = {
	Stream: require('CompositeStream'),
	stream: null,
	maxLength: 1e6,
	...
	ondata: function(chunk){
		...
		if( this.isTooLarge(this.bytesReceived) ){
			this.stream.emit('error', this.createTooLargeError(this.bytesReceived, this.byteLimit));
		}
		else if( this.hash ){
	...
	onend: function(){
		if( typeof this.bytesExpected == 'number' && this.bytesExpected != -1 && this.bytesReceived != this.bytesExpected ){
			this.stream.emit('error', this.createBadSizeError(this.bytesReceived, this.bytesExpected));
		}
		else if( this.hash && this.md5 != this.hash.digest() ){
			this.stream.emit('error', this.createBadDigestError(this.hash.digest(), this.md5));
		}
	},	
