/*

for a page at path

1. try to load a file at path ending by .page
2. try to load a file named path + '/' + index.page
3. repeat for all parent directories

examples:

resolve('/') -> 'index.page'
resolve('/index') or resolve('/index.json') -> 'index.page' or 'index/index.page'
resolve('/user/32/edit') ->
	'user/32/edit.page' or 'user/32/edit/index.page',
	'user/32.page' or 'user/32/index.page',
	'user.page' or 'user/index.page'

*/

var Resolver = {
	fileSystem: require('fs'),
	Path: require('path'),

	indexFileName: 'index',
	extension: 'page',
	sep: '/',

	path: null,

	init: function(path, relative){
		this.path = path;
		this.relative = relative;
	},

	isFile: function(path, fn){
		path = this.relative + path;

		this.fileSystem.stat(path, function(error, stat){
			if( error ){
				if( error.code === 'ENOENT' ){
					fn.call(this, null, false);
				}
				else{
					fn.call(this, error);
				}
			}
			else if( stat.isFile() || stat.isFIFO() ){
				fn.call(this, null, true);
			}
			else{
				fn.call(this, null, false);
			}
		});
	},

	loadAsFile: function(path, fn){
		var extension = this.Path.extname(path);
		
		// remove the extension (index.json should redirect to index.page and outputs json)
		if( extension ) path = path.slice(0, path.lastIndexOf('.'));
		// add the page extension
		if( this.extension ) path+= '.' + this.extension;

		this.isFile(path, function(error, is){
			if( error ){
				fn.call(this, error);
			}
			// on a trouvé le fichier
			else if( is ){
				fn.call(this, null, path);
			}
			// on a pas trouvé
			else{
				fn.call(this);
			}
		});
	},

	loadAsDirectory: function(path, fn){
		this.loadAsFile(path + this.sep + this.indexFileName, fn);
	},

	load: function(path, fn){
		this.loadAsFile(path, function(error, resolved){
			if( error ){
				fn.call(this, error);
			}
			else if( resolved ){
				fn.call(this, null, resolved);
			}
			else{
				this.loadAsDirectory(path, fn);
			}
		});
	},

	pagePaths: function(start){
		start = start.slice(1); // remove first '/'

		var Path = this.Path, parts = start.split(this.sep), dirs, dir, i, part;

		i = parts.length - 1;
		dirs = [];
		while( i > 0 ){
			part = parts.pop();
			dir = Path.join.apply(Path, parts);
			dirs.push(dir);

			i--;
		}

		return dirs;
	},

	loadPages: function(path, fn){
		var dirs = this.pagePaths(path), i = 0, j = dirs.length, dir;

		function nextdirectory(){
			if( i < j ){
				dir = dirs[i];
				this.load(dir, function(error, resolved){
					if( error ){
						fn.call(this, error);
					}
					else if( resolved ){
						fn.call(this, null, resolved);
					}
					else{						
						nextdirectory.call(this);
					}
				});
			}
			else{
				fn.call(this);
			}

			i++;
		}

		nextdirectory.call(this);
	},

	resolve: function(fn){
		// load as file or as directory
		this.load(this.path, function(error, resolved){
			if( error ){
				fn.call(this, error);
			}
			else if( resolved ){
				fn.call(this, null, resolved);
			}
			else{
				// if not found try to load parent directories
				this.loadPages(this.Path.dirname(this.path), fn);
			}
		});
	}
};

module.exports = Resolver;