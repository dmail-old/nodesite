...
var Page = proto.create({
	...
	createHandler: function(fn, bind){
		return new PageHandler(this, fn, bind);
	},
	...
	handlePageResult: function(result){
		if( !result || !('body' in result) ) return result;

		var template = this.createTemplate(null, result.body);
		debug('auto creating the template of page', this.path);

		return template.render().catch(function(error){
			// template doesn't exist, send row data
			if( error && error.code === 'ENOENT' ){
				debug('template not found', template.path);
				return result;
			}
	...
	createTemplate: function(path, params){
		...
		return new this.Template(path, params);
	},
