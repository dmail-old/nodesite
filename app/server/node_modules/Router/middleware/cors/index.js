...
module.exports = {
		...
		sendCORS: function(){
			this.populate({
				...
				headers: {
					...
					'access-control-allow-methods': this.methodsAllowed.join(', '),
					'access-control-allow-headers': this.headersAllowed.join(', '),
	...
	handle: function(router){
		if( router.request.is('options') ){
			router.response.sendCORS();
		}
		else{
			router.next();
		}
