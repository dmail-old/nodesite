...
var Response = proto.extend(PassThrough, {
	...
	prefetch: function(prefetcherPrototype){
		...
		if( this.prefetchers === null ){
			this.dataBuffer = [];
			this.prefetchers = [];
			debug('unpipe response to prevent response.write() & response.end()');
	...
	useStream: function(stream){
		if( stream != this ){
			...
			stream.pipe(this);
			//console.log('data written to stream are piped to the current stream itself piped to the response');
		}
		this.stream = stream;
	},

	pipeBody: function(body){
		return body.pipe(this.stream);
	},

	writeBody: function(body, encoding){
		debug('write into stream');
