...
var DBFileTemplate = proto.extend.call(FileTemplate, {
	name: null,
	partPrototype: require('./FileTemplate/JSONPart'),
	...
	findPart: function(selector){
		...
		selector = Selector.create(selector);
	...
	findAll: function(selector, callback, bind){
		/* selector can be like :
		{
			selector: selector,
			limit: Number,
			sortBy: String, CommaSeparatedString, Array[String | Function, [Number], ...]
			map: function(){},
			//reduce: function(){}
		}
		*/
		...
		this.callWhenReaded(function(error){
			if( error ){
				...
				var found = [], parts = this.parts, i = 0, j = parts.length, part;

				for(;i<j;i++){
					part = parts[i];
					if( selector.filter(part.item, i, part) ){
						if( selector.map ) part = selector.map(part);
						found.push(part);
						if( selector.limit && found.length >= selector.limit ){
							break;
						}
					}
				}
				...
				if( selector.sortBy ){
					found.sort(sortBy.createComparer(selector.sortBy));
				}

				this.reply(callback, bind, null, Cursor.create(found));
			}
		});
	},
