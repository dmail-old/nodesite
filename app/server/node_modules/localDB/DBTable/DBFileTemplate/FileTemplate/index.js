...
var FileTemplate = proto.extend.call(Emitter, {
	Part: require('./Part'),
	fileSystem: require('fs'),

	path: null,				// path to the file
	state: 'closed',		// 'closed','opening','opened','reading','readed','writing','closing'
	...
	constructor: function(path, encoding){
		...
		FileTemplate.super.constructor.call(this);
	},
	...
	open: function(callback, bind){
		...
		this.once('open', function(error, fd){
			if( error ){
				...
				this.state = 'opened';
				this.fd = fd;
				this.separatorBuffer = new Buffer(this.separator, this.encoding);
				this.Part.encoding = this.encoding;
				//this.parts = [];
	...
	createPart: function(){
		return this.Part.create.apply(this.Part, arguments);
	},

	indexOf: function(buffer, search, from){
		if( typeof search == 'string' ) search = new Buffer(search);

		var i = Number(from) || 0, j = buffer.length, k = 0, l = search.length, index = -1;

		while( i < j ){
			if( buffer[index] === search[k] ){
				if( index === -1 ) index = i;
				k++;
				if( k === l ) break;
			}
	...
	splitBuffer: function(buffer){
		var byte = 0, index = 0, separator = this.separatorBuffer, partBuffer, part, parts = [];

		while( true ){
			index = this.indexOf(separator, byte);
			part = this.createPart();
			part.byte = byte;

			if( index === -1 ){
				part.buffer = buffer.slice(byte);
				break;
			}
			else{
				part.buffer = buffer.slice(byte, index);
				byte = index + separator.length;
			}
		}
		...
		function onstat(error, stat){
			...
			this.once('read', function(error, readed, buffer){
				if( error ){
					...
					this.state = 'readed';
					this.parts = this.splitBuffer(buffer);
					this.reply(callback, bind, null, this.parts);
				}
			});
	...
	listBufferFrom: function(index){
		var parts = this.parts, i = index, j = parts.length, list = [];

		for(;i<j;i++){
			if( i !== index ){
				list.push(this.separatorBuffer);
			}
			list.push(parts[i].buffer);
		}
	...
	isPart: function(part){
		return this.Part.isPrototypeOf(part);
	},
	...
	replacePart: function(oldPart, part, callback, bind){
		...
		if( diff === 0 ){
			...
			var bufferList = this.listBufferFrom(index + 1);
			bufferList = [writeBuffer].concat(bufferList);
			writeBuffer = Buffer.concat(bufferList);
			this.truncateThenWrite(writeByte, writeBuffer, onsuccess);
		}
	...
	removePart: function(part, callback, bind){
		...
		this.truncateThenWrite(byte, Buffer.concat(this.listBufferFrom(index + 1)), function(error){
			if( error ){
				return this.reply(callback, bind, error);
			}
