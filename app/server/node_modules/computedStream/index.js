...
function ComputedStream(options){
	this.streams = [];
	var stream = Duplex.call(this, options);
	return stream;
}
util.inherits(ComputedStream, Duplex);

ComputedStream.prototype.resolving = false;
ComputedStream.prototype._read = ComputedStream.prototype.read;
ComputedStream.prototype._write = function(chunk){ this.push(chunk); };
ComputedStream.prototype.chain = function(stream, isHead){
	if( this.resolving ){
...
ComputedStream.prototype.resolve = function(){
	if( this.resolving ){
		return false;
	}
