...
getContent devrait être asynchrone
...
var HTMLPage = {
	...
	ScriptTemplate: require('MustachePercentTemplate').extend({
		encode: function(value){
			return JSON.stringify(value, function(value){
	...
	renderStyleTag: function(name){
		return this.setTagUrl(this.tags.style, 'css/' + name + '.css');
	},

	renderStyleTags: function(names){
		var output = [];

		names.forEach(function(name){
			output.push(this.renderStyleTag(name));
		}, this);

		return output.join('\n\t');
	},

	renderScriptTag: function(name){
		if( !name.endsWith('.js') ) name+= '.js';

		return this.setTagUrl(this.tags.script, name);
	},

	renderScriptTags: function(names){
		var output = [];

		names.forEach(function(name){
			output.push(this.renderScriptTag(name));
		}, this);
