...
var Page = proto.create({
	...
	getPageResult: function(){
		...
		if( cache ){
			...
			this.response.serverResponse.once('finish', function(){
				cache.set(this.request, this.response);
			}.bind(this));
		}
	...
	createTemplatePromise: function(path, params){
		...
		return tpl.render().then(function(result){
			// default text/html
			if( false === this.response.hasHeader('content-type') ){
				this.response.setHeader('content-type', config.getMimetype(tpl.path));
			}
			return result;
