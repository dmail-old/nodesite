...
var FileTemplate = Emitter.extend({
	...
	open: function(callback, bind){
		...
		this.once('open', function(error, fd){
			if( error ){
				...
				this.Splitter.separator = this.separatorBuffer[0];
				this.Splitter.Part.encoding = this.encoding;
				this.parts = [];
		...
		function onstat(error, stat){
			...
			this.once('read', function(error, readed, buffer){
				if( error ){
					...
					var splitter = this.Splitter.new(buffer);
					this.parts = splitter.split();
	...
	isPart: function(part){
		return this.Splitter.Part.isPrototypeOf(part);
	},
	...
	appendPart: function(part, callback, bind){
		...
		if( index === 1 && lastPart.buffer.length === 0 ){
			...
			byte = lastPart.index + lastPart.buffer.length + this.separatorBuffer.length;
