...
var ComputedStream = proto.extend(Transform, {
	constructor: function(){
		Transform.apply(this, arguments); // or Dupplex.apply(this, arguments)
		this.streams = [];
	...
	*/
	...
	_resolve: function(){
		debug('piping all computed stream');

		var stream, streams = this.streams, i = streams.length, prevStream = this, state;
	...
	removeAutoResolve: function(){
		this.removeListener('addListener', this.onAddListener);
		this.write = Transform.prototype.write;
	},
	...
	resolve: function(){
		...
		this.resolved = true;
		this.removeAutoResolve();
		this._resolve();

		return true;
	},

	// resolve for the first write (because of readableStream.pipe(computedStream) or write() call)
	write: function(){
		this.resolve();
		return Transform.prototype.write.apply(this, arguments);
	},

	// resolve when 'data' listener is added to computedStream
	...
	chain: function(stream, isHead){
		if( this.resolved ){
			throw new Error('computedStream is resolved no more stream can be chained');
		}
