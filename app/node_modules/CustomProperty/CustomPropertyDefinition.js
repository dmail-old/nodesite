...
var CustomPropertyDefinition = proto.extend({
	Getters: {
	...
	createParent: function(object){
		var parent = this.constructor.create(object, this.name);
		return parent;
	},

	createChild: function(object){
		var child = this.constructor.create(object, this.name);
		var childDescriptor = {};
	...
	addListener: function(fn, bind){
		if( this.notifier === null ){
			this.notifier = this.Notifier.create();
		}
		...
		if( this.descriptor === null ){ // there is no descriptor
			if( !Object.prototype.hasOwnProperty.call(this.object, this.name) ){
				...
				if( proto !== null && this.filterParents.indexOf(proto) === -1 ){
					...
					if( this.parent === null ){
						this.parent = CustomPropertyDefinition.create(proto, this.name).define();
					}
	...
	fromObject: function(object, name){
		if( !name ) throw new TypeError('property name expected');
...
var API = {
	...
	defineCustomProperty: function(object, name, descriptor){
		return CustomPropertyDefinition.create(object, name).define(descriptor);
	},

	addPropertyListener: function(object, name, fn, bind){
		var customDefinition = CustomPropertyDefinition.fromObject(object, name);
		if( customDefinition === null ){
			customDefinition = CustomPropertyDefinition.create(object, name);
		}
