...
var StringTemplate = proto.create({
	...
	replaceNull: function(expression){
	...
	replaceArray: function(expression, array, index){
	...
	exec: function(object){
		...
		var expressions = this.expressions, i = 0, j = expressions.length, values = new Array(j), replace;

		if( object == null ){
			replace = this.replaceNull;
		}
		else if( object instanceof Array ){
			replace = this.replaceArray;
		}
		else{
			replace = this.replaceObject;
		}

		for(;i<j;i++){
			values[i] = replace.call(this, expressions[i], object, i);
		}
