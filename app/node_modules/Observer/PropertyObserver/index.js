...
var PropertyObserver = proto.create({
	...
	childSetter: function(child, value){
		...
		if( this.protoObserver ) this.protoObserver.unobserve(this.onProtoChange);
	...
	open: function(){
		...
		if( descriptor ){
			...
			this.parentProto = Object.getPrototypeOf(object);
			if( this.parentProto && this.parentProto != Object.prototype ){
				this.protoObserver = PropertyObserver.new(this.parentProto, this.name)
				this.protoObserver.observe(this.onProtoChange);
			}
			descriptor = {
	...
	unobserve: function(fn){
		if( fn ){
			...
			if( index !== -1 ){
				if( fn == this.onProtoChange && this.protoObserver ){
					this.protoObserver.unobserve(this.onProtoChange);
				}
