...
Table.implement({
	...
	nextFree: function(key, value, count){
		switch(key){
			case 'name': return (count ? value.substr(0, value.lastIndexOf(' ')) : value) + ' ('+(count + 2)+')';
			case 'id': default: return value + 1;
		}
	},
...
Table.implement({
	getRule: function(key, name){
		...
		switch(arguments.length){
			case 0: return rules;
			case 1: return rules[key] || null;
			case 2: rules = rules[key]; return rules ? (rules[name] || null) : null;
		}
	...
	hasRule: function(key, name){
		switch(arguments.length){
			case 0: return Boolean(this.getRule());
			case 1: return Boolean(this.getRule(key));
			case 2: var rules = this.getRule(key); return rules && name in rules;
		}
	...
	matchRules: function(item, callback){
		var keys = Object.keys(item), key, value, matchError, i = 0, j = keys.length, key, count = 0;
		...
		function nextTable(){
			if( i < j ){
				...
				switch(action){
					...
					case 'update':
						var prop = {}; return prop[key] = newid;
